...
import { CohetesController } from './../controller/controller';

window.addEventListener('load', onPageLoad);

let cohetesController: CohetesController;
...
function printRocket(codigo: string) {
...
function accelerateRocket(codigo: string) {
  let cohete = cohetesController.getCohete(codigo);
  if (cohete) {
    cohete.acelerar();
  }
...
function breakRocket(codigo: string) {
  let cohete = cohetesController.getCohete(codigo);
  if (cohete) {
    cohete.break();
  }
